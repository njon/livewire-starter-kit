    ...
    <div class="col-lg-8">
        ...
        <div class="card border-0  rounded-4 overflow-hidden">
            <div class="card-body p-5 text-center">
                ...
                <!-- Main Heading -->
                <h1 class="display-5 fw-bold text-success mb-3">{{ __('Order Confirmed!') }}</h1>
                <p class="lead mb-4">{{ __('Your experience is being prepared with care') }}</p>

                <!-- Order Details -->
                <div class="bg-light p-4 rounded-3 mb-4">
                    <div class="row">
                        <div class="col-md-6 mb-3 mb-md-0">
                            <h3 class="h5 mb-2 text-start">{{ __('Order Summary') }}</h3>
                            <ul class="list-unstyled text-start">
                                <li class="mb-2"><strong>{{ __('Order #:') }}</strong> {{ $order->id }}</li>
                                <li class="mb-2"><strong>{{ __('Date:') }}</strong>
                                    {{ $order->created_at->format('F j, Y') }}</li>
                                <li class="mb-2"><strong>{{ __('Total:') }}</strong> {{ $order->total->formatted }}</li>
                            </ul>
                        </div>
                        <div class="col-md-6">
                            <h3 class="h5 mb-2 text-start">{{ __("What's Next") }}</h3>
                            <ul class="list-unstyled text-start">
                                <li class="mb-2"><span class="badge bg-primary me-2">1</span>
                                    {{ __('Instant booking confirmation') }}</li>
                                <li class="mb-2"><span class="badge bg-primary me-2">2</span>
                                    {{ __('Details sent to your email') }}</li>
                                <li><span class="badge bg-primary me-2">3</span>
                                    {{ __('Prepare for an unforgettable experience!') }}</li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <div class="alert alert-success bg-opacity-10 border-0 mb-4 py-4">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1 ms-3">
                            <h4 class="alert-heading mb-3" style="margin-left: -50px;">
                                {{ __('Get Ready for Adventure!') }}</h4>
                            <p class="mb-0">{!! __('Your <strong>Air Jordan Winter Experience</strong> is now confirmed.
                                Our team is preparing everything to ensure you have an amazing time. Expect your
                                detailed instructions via email shortly.') !!}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-5 pt-3 border-top">
                    <p class="text-muted mb-3">{{ __('Share your excitement!') }}</p>
                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card border-0 shadow-sm mt-4 rounded-4 overflow-hidden text-start">
                        <div class="card-body p-4">
                            <h3 class="h5 mb-3 text-start mb-5">{{ __('Your Upcoming Experience') }}</h3>
                            ...
                            <div class="d-flex flex-column flex-md-row gap-3">
                                <img src="https://images.unsplash.com/photo-1544161515-4ab6ce6db874?ixlib=rb-1.2.1&amp;auto=format&amp;fit=crop&amp;w=300&amp;q=80"
                                    alt="{{ __('Air Jordan Winter Experience') }}" class="rounded-3 object-fit-cover"
                                    width="120" height="120">
                                <div>
                                    <h4 class="h6 mb-1">
                                        {{ $line->purchasable->product->translateAttribute('name') }}</h4>
                                    <p class="small text-muted mb-2">{{ __('2 participants • 45 minutes') }}</p>
                                    <p class="mb-2"><span
                                            class="badge bg-success bg-opacity-10 text-success">{{ __('Confirmed') }}</span>
                                    </p>
                                    <p class="small mb-0">{{ __('Athens, Thessaloniki • June 25, 2024') }}</p>
                                </div>
                            </div>
                            @endforeach
                        </div>
...
<div class="alert alert-light border mb-4 text-start d-none">
    <h3 class="h6 text-muted mb-3">{{ __('Billing Information') }}</h3>
